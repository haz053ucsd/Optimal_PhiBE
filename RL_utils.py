...
def mat_Q_cal_stochastic_RL_better(traj_mat, act_mat, bases_Q, dt, beta):
    # computes the matrix for Q
    # traj_mat shape (m, I), act_mat shape (m, I), bases_Q takes (m, I), (m, I) returns (dim_bases_Q, m, I)
    #return shape (M+1, M+1)
    ...
    # Compute b
    b = a - 1 / (beta * dt + 1) * bases_Q(traj_mat_next, act_mat_next).permute(1, 2, 0)
...
def mat_V_cal_stochastic_RL_better(traj_mat, bases_V, dt, beta):
    # computes the matrix for Q
    # traj_mat shape (m, I), act_mat shape (m, I), bases_Q takes (m, I), (m, I) returns (dim_bases_Q, m, I)
    #return shape (M+1, M+1)
    ...
    # Compute b
    b = a -1 / (beta * dt + 1) * bases_V(traj_mat_next).permute(1, 2, 0)
...
def b_cal_Q_RL_better(traj_mat, act_mat, reward_mat, bases_Q, dt, beta):
    # traj_mat shape (m, I), reward_mat shape (m, I), bases takes (m, I), (m, I) returns (M+1, m, I)
    # returns (M+1)
    ...
    ans = weighted_bases.sum(dim=(0, 1)) * dt / (beta * dt + 1)
...
def b_cal_V_RL_better(traj_mat, reward_mat, bases_V, dt, beta):
    # traj_mat shape (m, I), reward_mat shape (m, I), bases takes (m, I), (m, I) returns (M+1, m, I)
    # returns (M+1)
