...
def phibe_finder_1D_LQR(
    ...
    if Q_method == "GD" and Q_init.shape[0] != dim_bases_Q:
        raise ValueError("Invalid Initialization of Q coefficient.")
    if true_V.shape[0] != 3:
        raise ValueError("True should be the coefficients under basis 1, s, s^2.")
    ...
    for _ in tqdm(range(num_iter), desc=f"Running Optimal Phibe of order {order} using {Q_method}"):
        ...
        if Q_method == "GD":
            ...
            ValueError("Invalid method. Supported methods are GD or Galerkin.")
...
def phibe_finder_2D_LQR(
...
) -> Tuple[List[torch.Tensor], List[float], Callable]:
    """
    ...
    Returns:
        Tuple containing:
            ...
            - List of distances between optimal and approximated value functions at each iteration,
    """
    if Q_method != "GD" and Q_method != "Galerkin":
        raise ValueError("Invalid method. Supported methods are GD or Galerkin.")
