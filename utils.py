...
def true_V_eval_2D(A, B, b, R, Q, beta, sigma):
    ...
    using symbolic computation and returns them as PyTorch tensors.

    Args:
        A (torch.Tensor): 2x2 system dynamics matrix.
        B (torch.Tensor): 2x2 control matrix.
        b (torch.Tensor): 2x2 policy gain matrix.
        c (torch.Tensor): 2x1 policy offset vector.
        R (torch.Tensor): 2x2 positive definite control cost matrix.
        Q (torch.Tensor): 2x2 positive definite state cost matrix.
        beta (float): Discount rate.
        sigma (float): Noise coefficient.

    Returns:
        tuple: (P, p, p0) as PyTorch tensors where
               - P is a 2x2 torch.Tensor.
               - p is a 2x1 torch.Tensor.
               - p0 is a scalar torch.Tensor.
    """
    ...
    if beta != 0:
        ...
        if not solution:
            # raise ValueError("No solution found for the given LQR system.")
...
def is_stabilizable(A, B):
    ...
    A system is stabilizable if for every eigenvalue λ of A with Re(λ) >= 0,
    the matrix [A - λI, B] has full rank.
    """
...
def is_detectable(A, C):
    ...
    A system is detectable if for every eigenvalue λ of A with Re(λ) >= 0,
    the matrix [A - λI; C] has full rank.
    """
    eigvals, eigvecs = torch.linalg.eig(A)  # Compute eigenvalues of A
    n = A.shape[0]
